...
#description:    NDK cross compile build script template for autoconf build system	
#author:         agnostic-apollo
#usage:          To be used with `android_ndk_cross_compile_build_automator.sh`.
#date:           1-Aug-2019
#versions:       1.0
#license:        sMIT License
...
mkdir build
if [ $? -ne 0 ]; then
	echo "Failure to create build dir for $PROJECT for $ARCH_SRC"
	exit 1
fi
...
#if your project provides its own cmake or build script, run it
